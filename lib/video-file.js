...
import {strToMilliseconds as strToMs, isWindows} from './util.js';
...
export default class VideoFile {
  ...
  _detectCrop() {
    ...
    .then((output) => {
      ...
      if (useCommands.length === 0) {
        ...
        throw new TranscodeError(`Crop detection returned conflicting results.`, this.fileName, useCommands.join('\n'));
      }
  ...
  _encodeStatus(didFinish) {
    ...
    .then((log) => {
      let matches = log.trim().match(handbrakeLog);
      this.encodeTime = matches !== null ? strToMs(matches[1]) : false;
    });
  }
