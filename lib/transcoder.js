...
function transcode(filePath) {
  ...
  .then(function () {
    return childPromise('detect-crop', [path.relative(curDir, filePathNorm)], fileName, curDir, !options['debug']);
  })
  .then(function (res) {
  ...
  .then(function (output) {
    ...
    if (options['dry-run']) {
      ...
      .then(function (log) {
        ...
        say('[Time: ' + totalTime + '] ' + destRelPath, 'write');
      });
    }
  })
  .catch(function (err) {
